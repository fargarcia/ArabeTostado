import { Game, Hand as HandModel, Minion as MinionModel, MinionContainer, Player } from "models";
...
const Battlefield = ({ isOponent, player, oponent }: Props) => {
    ...
    const transitions = useTransition(battlefield.minions.map(minion => minion.id), {
        from: { opacity: 0 },
        enter: { opacity: 1 },
        leave: { opacity: 0 },
        delay: 200
    ...
    const renderMinion = ({ style, item: id, i }: RenderProps) => (
        <animated.div style={style} className={styles.container} key={`minion:${id}`}>
            ...
            {i < (battlefield.minionsCount() - 1) && (
                <SpaceBetween position={POSTION.CENTER} index={i + 1} />)
            }
        </animated.div>
    )
