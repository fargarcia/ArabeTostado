...
export class CardContainer {
  ...
  constructor(cards?: Card[] | CardContainer) {
    this._cards = !cards ? [] : Array.isArray(cards) ? cards : cards.cards.map((card) => new Card(card));
  }
  ...
  public removeCard = (removedCard: Card) =>
    this.cards.splice(this._cards.indexOf(this._cards.find((card) => card.id === removedCard.id)!), 1);
